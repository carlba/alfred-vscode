...
function getIcon(project) {
  const iconPaths = project.paths.map(projectPath => path.join(projectPath, 'icon.png'));
...
function getChannelPath(appdata = '', vscodeEdition = 'code') {
  if (vscodeEdition === 'code-insiders' && fs.existsSync(''.concat(appdata, '/Code - Insiders'))) {
    return 'Code - Insiders';
  }

  if (vscodeEdition === 'codium' && fs.existsSync(''.concat(appdata, '/VSCodium'))) {
    return 'VSCodium';
  }
